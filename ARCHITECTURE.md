...
**Actor identity is now bridged, not shared.** commons-crew's approval endpoint checks workspace membership, and initially only recognized its own seeded identity (`user_primary`) — every commons-board decision would have been attributed to the same placeholder regardless of who actually decided. `ensureBoardMemberIdentity` closes this using commons-crew's own existing user/membership system (`POST /api/users`, `POST /api/workspaces/:id/memberships` with an `approval_decision` permission) — no new commons-crew capability was needed, just a commons-board-side bridge: a real commons-crew user + "supporting" membership per commons-board admin, namespaced by org so two orgs' same user id can't collide, created once and reused. Falls back to `user_primary` only if the bridge itself can't run. Live-verified: the bridged identity actually deciding a real approval, not just existing as a record.

One thing still genuinely open: nothing in commons-board's normal request lifecycle calls the dispatch mechanism automatically yet — an admin/operator has to trigger it explicitly per request, and reconciling it with the existing direct-LLM `chair-reasoning.ts` path (which this doesn't touch) is a deliberate, separate product decision, not made here. Until that's addressed, the hundreds of line-level specialists the catalog contains are technically reachable through a real, verified, identity-correct mechanism, but not exercised by default in day-to-day use.
...
**Current gap:** commons-board now calls `pa.createChairRun` once per chair at onboarding, and can dispatch real work to that run via a proposal it explicitly approves, decided by a real bridged commons-board identity rather than a shared placeholder (see commons-board, above). One primitive this uncovered and fixed: a chair's seeded delegation approval is one-shot by design (an approval authorizes one specific act, not a standing blank check), so a long-lived chair had no way to delegate a *second* time — `pa.requestDelegationApproval` (`POST /api/runs/:runId/delegation-approvals`) seeds a fresh one on request, verified live across multiple sequential dispatches to the same chair. `search_artifacts` (`class_a`, read-only, no approval needed) is the commons-crew side of artifact-commons matching — verified live against the real artifact-commons repo. What's still open: commons-board's normal request lifecycle doesn't call the dispatch mechanism automatically yet, and nothing calls `search_artifacts` automatically before a task reaches for build capability either — both tools work, neither is invoked on its own initiative.
...
**Both questions are now ratified in [GOVERNANCE.md](GOVERNANCE.md).** Per-organization authority is already answered by a mechanism that exists today — commons-board's autonomy tiers — and is now stated as policy rather than left implicit in the autonomy-tier code. Platform-level authority — what it takes for something to be trusted enough to be treated as binding across the whole commons, not just within one org's deployment — was the open question; `GOVERNANCE.md` grounds it in the mechanisms that already exist (labor-commons-curator's certification gate, commons-keeper's independent review loop) and states who has standing to certify, how disagreement is handled, and how a certification gets revoked. artifact-commons will need the equivalent certification gate before an artifact is trusted enough to be surfaced as a match rather than merely discovered — `GOVERNANCE.md`'s model is written to extend to it directly (the repo exists now; the gate itself doesn't), not to be re-decided.
