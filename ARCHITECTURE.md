...
A real, already-generated seed for that second axis exists and is unmigrated: 260 specialist definitions, hand-curated across 22 functional domains (finance, HR, legal-and-compliance, marketing, sales-and-revenue, operations, product, and others), originally built for mother-board (this stack's predecessor) via a scripted issue-driven pipeline, fully executed and closed out. They currently sit in the `jkm-agents` repo in the legacy `manifest.yaml` shape, untouched since — no overlap with labor-commons' own ~500 open specialist-generation issues (a separate, NAICS-only pipeline), no reference to them anywhere in labor-commons.

**Planned, not yet built:** migrate those 260 into labor-commons as `catalog/function-overlays/{domain}/{specialist-slug}/spec.yaml` — the same shape as `naics-overlays`, one directory level per domain instead of per industry, converted from `manifest.yaml` to the current `spec.yaml` schema. Domain folder names drop the source's `director-of-` prefix (`director-of-finance` → `finance`) since the folder names a function, not a job title, matching how industry folders are named. Three downstream consumers already hardcode the `naics-overlays` path specifically and will need widening once this exists: foundation-site's live catalog-count script, commons-crew's `LocalCatalogService` catalog scanner, and at least one (possibly more) of commons-board's own labor-commons readers.
...
**Current gap:** commons-board's existing implementation duplicates this — it has its own direct SQL-based resolver against labor-commons, built before commons-crew's recursive shape existed. That resolver should retire in favor of treating chairs as commons-crew instances like every other layer, now that commons-crew supports recursive delegation (see commons-crew, below — this is built and merged). Until commons-board is wired to it, commons-board's reach into labor-commons is also limited to whatever its own resolver maps to the chair layer — the hundreds of line-level specialists the catalog actually contains are unreachable through commons-board today.

A second, independent reason chair-matching produces poor results: it's not just a routing problem, it's a missing-data problem. commons-board's resolver searches `naics-overlays` — industry-vertical specialists — for something that needs the function-overlays axis described in labor-commons, above. A finance chair needs generic FP&A/treasury/AP-AR depth, not an industry-specific operational specialist; that specialty type doesn't exist in the catalog yet at all, so no amount of better matching logic would have fixed it. Both problems need fixing — the resolver and the missing axis — independently.
...
**Built and merged to commons-crew's `main`.** `delegate_to_child` is a real action tool, going through the same governed propose/approve/execute loop as any other side-effecting action. Single-hop and multi-hop delegation both work end to end (chair → director → department → worker), verified by real tests against the actual public API, not mocked. `pa.createChairRun` registers a root run as a specific chair (a fixed v1 role set — finance, legal, HR, marketing, operations, product) for a specific org, pre-authorized to delegate immediately, with that org context propagating down everything it spawns. Full detail in commons-crew's own `docs/architecture.md`.

**Current gap:** two things, both on the *consumer* side now, not commons-crew's own mechanism. First, nothing in commons-board calls any of this yet — its onboarding flow still uses its pre-existing direct resolver against labor-commons to staff chairs, with no awareness commons-crew exists; wiring that up is a cross-repo integration that hasn't started. Second, commons-crew's own catalog scanner (`LocalCatalogService`) only scans `catalog/naics-overlays` — once labor-commons gains the `function-overlays` axis (see labor-commons, above), this scanner needs widening too, or chairs assembling specialists for their own domain will still miss the catalog half built specifically for them.
...
- **labor-commons** — the catalog
- **commons-board** — governed authority
- **commons-crew** — recursive delegation (built and merged; commons-board not yet wired to call it)
